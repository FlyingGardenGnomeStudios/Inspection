...
namespace ScrewTurn {
	...
	public class DropDownPanel : System.Windows.Forms.Panel {
		...
		// #############################################################
		private int animationStepDelay = 5; // TODO: compute this value dinamically, in function of Height
		...
		// Header ContextMenu
		private ContextMenu ctxHeader;
		private bool enableContextMenu = true;
		private MenuItem itmExpand;
		private MenuItem itmMoveable;
		private MenuItem itmRestoreLocation;
		...
		public DropDownPanel() {
			...
			// Create ContextMenu
			ctxHeader = new ContextMenu();
			if(expanded) {
				itmExpand = new MenuItem("Collapse Panel");
			}
			else {
				itmExpand = new MenuItem("Expand Panel");
			}
			itmExpand.Click+= new EventHandler(ctxHeader_ItemClick);
			itmMoveable = new MenuItem("Moveable");
			if(moveable) {
			...
			itmRestoreLocation = new MenuItem("Restore Home Location");
			itmRestoreLocation.Click += new EventHandler(ctxHeader_ItemClick);

			ctxHeader.MenuItems.Add(itmExpand);
			ctxHeader.MenuItems.Add(new MenuItem("-"));
			ctxHeader.MenuItems.Add(itmMoveable);
			ctxHeader.MenuItems.Add(itmRestoreLocation);
